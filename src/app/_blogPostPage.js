...
import BlogShareButton from './blog/[slug]/BlogShareButton';
...
function markdownToHtml(md = '') {
  ...
  html = html.replace(/:::startbox(?:\[(.*?)\])?\n([\s\S]*?)\n:::/g, (_m, title = '', body = '') => {
    const safeTitle = escapeHtml((title || 'Start here').trim());
...
function buildSubcategoryLinksHtml(title = '같은 주제의 글 더 보기', items = []) {
  ...
  return `<section class="md-subcatbox"><h3 class="md-subcatbox-title">${safeTitle}</h3><ul class="md-subcatbox-list">${items
    .map((item) => `<li class="md-subcatbox-item"><a class="md-link" href="/${escapeHtml(item.slug)}">${escapeHtml(item.title)}</a></li>`)
    .join('')}</ul></section>`;
}
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `${seoTitle || post.title} | ${SITE_NAME}`,
    description: summary,
            ...
            <div className="mb-8">
              ...
              <ul className="grid gap-2 md:grid-cols-2">
                {relatedPosts.map((item) => (
                  <li key={item.id}>
                    <Link href={`/${item.slug}`} className="text-[14px] text-[#0ABAB5] hover:underline">
                      {item.title}
                    </Link>
                  </li>
                ))}
